...
$$|G| = \sum_{i=1}^n d_i^2$$
where $d_i$ is either character degrees or square roots of conjugacy class sizes and $i$ indexes the total set of characters or conjugacy classes. After initial definitions, we will begin with a few characterizations based on integral values of $e$. Then, we will bound the order of a group by our parameter and characterize groups attaining this boundary.
...
This parameter is small for groups with a large conjugacy class. In particular, the smallest possible values of $e$ are attained only by groups with a conjugacy class that contains exactly half of the group's elements, the largest non-trivial conjugacy class possible relative to a group's order (\ref{boundaryclassification). Given the 

 Let $x \in G$ be such that $e = (c(x) - 1) \cdot \sqrt{k(x)}$. Consider the following:
\begin{align*}
e &= (c(x) - 1) \cdot \sqrt{k(x)} \\
 &= c(x)\frac{k(x)}{\sqrt{k(x)}} - \sqrt{k(x)} \\
 &= \frac{|G|}{\sqrt{k(x)}} - \sqrt{k(x)}\text{.}
\end{align*}
If we let $d = \sqrt{k(x)}$, we arrive at the equation $|G| = d (d + e)$. Since $k(x)$ is precisely the size of the conjugacy class containing $x$, for a fixed $G$, when the conjugacy class of $x$ is very large, $e$ is very small. This behavior is similar to Snyder's parameter with character degrees. As mentioned above, if we replace $d$ with the degree of an irreducible character, we obtain Snyder's original parameter. With this, we now follow Snyder's example and classify groups where $e$ is 0, 1, or 2.
